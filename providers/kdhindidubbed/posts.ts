...
/** A normal KDHindiDubbed page has at least some WordPress content markers. */
...
async function scrapePosts({
  ...
  try {
    ...
    if (catalog.length === 0 && openWebView) {
      const looksBlocked = looksLikeChallenge($) || !looksLikeWordPressPage($);
      if (looksBlocked) {
        console.log(`KDHindiDubbed challenge detected for ${url}, using solver...`);
        const retry = await solveWafAndRetry({ url, signal, axios, openWebView });
        if (retry) {
